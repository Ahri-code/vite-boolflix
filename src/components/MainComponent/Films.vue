    ...
    methods: {
        posterImg(index) {
            ...
            if (index.poster_path != null) {
                ...
                path = "../../assets/NO-MOVIE-POSTERS-02-03-003_1000.jpg";
                return path;
            }
        },
        starConverter(index) {
        ...
        <div class="flex gap-1 overflow-hidden">
            <div v-for="i in store.moviesArray[0]">
                <div class="flex fd-columns gap-1 relative">
                    <img :src="posterImg(i)">
                    <div class="p-05 c-white flex fd-columns gap-05 absolute movieInfo" @mouseenter="startScrolling">
                        <h3 class="ta-center">{{ i.title }}</h3>
                        <div class="flex jc-between">
                            <p>{{ i.release_date }}</p>
                            <p><span class="fi" :class="flagChooser(i)"></span> {{ i.original_language }}</p>
                        </div>
                        <p class="overflow-hidden"><small>{{ i.overview }}</small>
                        </p>
                        <p>{{ starConverter(i) }}</p>
                    </div>
                </div>
